...
class Visualizer:
    ...
    def __init__(self, env: Environment, task_dict):
        ...
        plt.ion()
        self.fig, self.ax = plt.subplots(figsize=(x_dim, y_dim)) # TODO - scale figure size by env subsample
    ...
    def _add_tasks_to_plot(self):
        ...
        task_y = []
        task_z = []
        for loc in self.task_dict.values():
            task_x.append(loc[0])
            task_y.append(loc[1])
            if not self.env.SLICE: task_z.append(loc[2])
    ...
    def _add_tour_lines_to_plot(self, agent_list: list[Agent]):
        ...
        for a in agent_list:
            ...
            end_loc = self.task_dict[a.action[1]]
            ...
            if len(a.schedule) > 0:
                ...
                for task_id in a.schedule:
                    end_loc = self.task_dict[task_id]
...
def set_up_visualizer_from_config(
    env: Environment, config_filepath: str
    ) -> Visualizer:
