...
class Environment:
    ...
    def __init__(self,
        ...
        self.time_steps = 60/time_step_size # divide hour by time step size to get number of steps
        ...
        self.SLICE = False
        if env_dimensions[2][0] == env_dimensions[2][1]: self.SLICE = True
        
        # Crop & thin topography data
        topography = xr.open_dataset(topography_file)
        x_coords = topography['xx'][env_dimensions[0][0]:env_dimensions[0][1]:x_thinning]
        y_coords = topography['yy'][env_dimensions[1][0]:env_dimensions[1][1]:y_thinning]
        z_coords = topography['zz'][env_dimensions[2][0]:env_dimensions[2][1]:z_thinning]
        ...
        for filename in flow_data_files:
            ...
            if not self.SLICE:
                u_vecs = [[row[env_dimensions[0][0]:env_dimensions[0][1]:x_thinning]
                            for row in plane[env_dimensions[1][0]:env_dimensions[1][1]:y_thinning]]
                            for plane in data['uu'][env_dimensions[2][0]:env_dimensions[2][1]:z_thinning]]
                v_vecs = [[row[env_dimensions[0][0]:env_dimensions[0][1]:x_thinning]
                            for row in plane[env_dimensions[1][0]:env_dimensions[1][1]:y_thinning]] 
                            for plane in data['vv'][env_dimensions[2][0]:env_dimensions[2][1]:z_thinning]]
                w_vecs = [[row[env_dimensions[0][0]:env_dimensions[0][1]:x_thinning]
                            for row in plane[env_dimensions[1][0]:env_dimensions[1][1]:y_thinning]]
                            for plane in data['ww'][env_dimensions[2][0]:env_dimensions[2][1]:z_thinning]]
            else:
                u_vecs = [[row[env_dimensions[0][0]:env_dimensions[0][1]:x_thinning]
                            for row in data['uu'][env_dimensions[2][0]][env_dimensions[1][0]:env_dimensions[1][1]:y_thinning]]]
                v_vecs = [[row[env_dimensions[0][0]:env_dimensions[0][1]:x_thinning]
                            for row in data['vv'][env_dimensions[2][0]][env_dimensions[1][0]:env_dimensions[1][1]:y_thinning]]]
                w_vecs = [[row[env_dimensions[0][0]:env_dimensions[0][1]:x_thinning]
                            for row in data['ww'][env_dimensions[2][0]][env_dimensions[1][0]:env_dimensions[1][1]:y_thinning]]]
        ...
        self.flow_data_idx = 0 #index of first file we are interpolating flow from (idx+1 for next file)
        self.current_flow_state = self.processed_flow_data[self.flow_data_idx] # current environment frame
        ...
        if len(self.processed_flow_data) > 1:
            self.flow_data_modifiers = self._update_flow_modifiers(self.processed_flow_data[self.flow_data_idx], 
    ...
    def _update_flow_modifiers(self, flow_hour1, flow_hour2):   
        ...
        """
        # Find differences
        u_diff = np.subtract(np.array(flow_hour2['u']), np.array(flow_hour1['u']))
        v_diff = np.subtract(np.array(flow_hour2['v']), np.array(flow_hour1['v']))
        w_diff = np.subtract(np.array(flow_hour2['w']), np.array(flow_hour1['w']))
    ...
    def get_local_flow(self, loc):
        ...
        if not self.SLICE:
            ...
            local_flow_x = self.current_flow_state['u'][local_z][local_y][local_x].values #z,y,x idx
            local_flow_y = self.current_flow_state['v'][local_z][local_y][local_x].values
            local_flow_z = self.current_flow_state['w'][local_z][local_y][local_x].values
    ...
    def step(self, agent_list):
        ...
        for a in agent_list:
            ...
            if a.action[0] == a.IDLE:
                ...
                new_loc = tuple(agent_loc[i] + scaled_travel_vec[i] for i in 
                                range(len(agent_loc)))
...
def make_environment_from_config(
    ...
    return Environment(topo_filepath,
                       tidal_fps,
                       dims,
                       agent_loc_dict,
                       thinning,
